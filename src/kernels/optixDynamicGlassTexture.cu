...
extern "C" __global__ void __raygen__rg() {
    ...
    const int num_samples = 1; // Define the number of samples per pixel
    float3 accumulated_color = make_float3(0.0f, 0.0f, 0.0f);
    for(int sample = 0; sample < num_samples; ++sample) {
        PerRayData prd;
        prd.accumulated_color = make_float3(0.0f, 0.0f, 0.0f);// Start with black
        prd.throughput = make_float3(1.0f, 1.0f, 1.0f);       // Full throughput
        prd.depth = 0;                                        // Initial depth
        unsigned int pixel_id = idx.y * params.width + idx.x;
        unsigned int seed = params.subframe_index * params.width * params.height + pixel_id;
        curand_init(seed, pixel_id, 0, &prd.state);

        trace(params.handle, eye, ray_dir, 0.00f, 1e16f, &prd);
        accumulated_color += prd.accumulated_color;
    }
    accumulated_color /= static_cast<float>(num_samples);

    params.frame_buffer[idx.y * params.width + idx.x] = make_color( accumulated_color );
}
